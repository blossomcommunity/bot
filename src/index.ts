	...
	if (commandName === "help" && args[0]) {
		...
		if (!command) {
			return message.reply(
				`This command does not exist. Use ${prefix}help to see all commands`
			);
		}

		const embed = new StandardEmbed(message.author).addField(
			"Description",
			command.description
		);
	...
	if (commandName === "help") {
		...
		const fields = entries
			.filter(x => {
				...
				if (
					inhibitors.includes(staff) &&
					message.member?.roles.cache.get(staffRole as string)
				) {
					return true;
				} else {
